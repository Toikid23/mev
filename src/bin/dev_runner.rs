...
async fn test_whirlpool_with_simulation(rpc_client: &RpcClient, payer: &Keypair, current_timestamp: i64) -> Result<()> {
    const POOL_ADDRESS: &str = "Czfq3xZZDmsdGdUyrNLtRhGc47cXcZtLG4crryfu44zE"; // VOTRE POOL WSOL-USDC
    const INPUT_MINT_STR: &str = "So11111111111111111111111111111111111111112"; // WSOL
    const INPUT_AMOUNT_UI: f64 = 0.05;
    ...
    // --- 2. PRÉPARATION DE LA SIMULATION (Correcte) ---
    ...
    // --- 3. EXÉCUTION & ANALYSE (Méthode Robuste par lecture de compte) ---
    println!("\n[3/3] Exécution de la simulation standard...");
    ...
    let sim_config = RpcSimulateTransactionConfig {
        ...
        accounts: Some(solana_client::rpc_config::RpcSimulateTransactionAccountsConfig {
            ...
            addresses: vec![
                user_destination_ata.to_string(),
            ],
        }),
        ..Default::default()
    };
    ...
    if difference_percent < 0.01 {
        println!("✅ SUCCÈS ! Le décodeur Whirlpool est parfaitement précis.");
    } else {
        println!("⚠️ ÉCHEC ! La différence est trop grande.");
    }

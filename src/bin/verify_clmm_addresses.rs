...
use mev::{
    config::Config,
    decoders::raydium_decoders::{clmm_pool, tick_array},
};
...
async fn main() -> Result<()> {
    println!("--- Lancement du VÉRIFICATEUR TOTAL D'ADRESSES ---");
    ...
    // On simule un utilisateur pour construire l'instruction
    ...
    let (_, required_tick_arrays) = pool.get_quote_with_tick_arrays(&input_mint_pubkey, 1000000, 0)?;

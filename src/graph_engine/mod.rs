...
use crate::decoders::raydium_decoders::{amm_config, clmm_config, tick_array, launchpad, global_config, clmm_pool, amm_v4, cpmm}; // Outils
    ...
    pub async fn hydrate_pool(&self, pool: Pool, rpc_client: &RpcClient) -> Result<Pool> {
        match pool {
            ...
            Pool::MeteoraDlmm(mut p) => {
                ...
                dlmm::hydrate(&mut p, rpc_client).await?;
